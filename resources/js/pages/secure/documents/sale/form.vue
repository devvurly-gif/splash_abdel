        ...
        <div class="bg-bg-elevated rounded-xl w-full max-w-6xl max-h-[90vh] overflow-y-auto shadow-xl border border-surface-border">
            <div class="flex justify-between items-center p-6 border-b border-surface-border sticky top-0 bg-bg-elevated z-10">
                <h2 class="text-xl font-semibold text-text-primary m-0">
                    {{ document ? $t('document.sale.edit') : $t('document.sale.create') }} - {{ $t(`document.sale.${type}`) }}
                </h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        >
                            ...
                            <option v-for="partner in customers" :key="partner.id" :value="partner.id">
...
onMounted(async () => {
    // Load customers (partners with type customer or both)
    await partnerStore.fetchPartners({ type: 'customer', status: 1 });
    customers.value = partnerStore.partners.filter(p => p.isCustomer || p.type === 'both');
