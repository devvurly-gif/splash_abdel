<template>
    <div class="w-full">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-text-primary m-0">{{ $t('document.stock.title') }}</h1>
            <div class="flex gap-3">
                <select v-model="documentType" @change="handleTypeChange" class="py-2.5 px-4 border-2 border-surface-border rounded-lg text-sm cursor-pointer bg-bg-primary text-text-primary focus:outline-none focus:border-border-focus focus:ring-2 focus:ring-accent-primary-light">
                    <option value="adjustment">{{ $t('document.stock.adjustment') }}</option>
                    <option value="manual_entry">{{ $t('document.stock.manualEntry') }}</option>
                    <option value="manual_exit">{{ $t('document.stock.manualExit') }}</option>
                    <option value="transfer">{{ $t('document.stock.transfer') }}</option>
                </select>
            ...
            <table class="w-full border-collapse">
                <thead class="bg-bg-secondary">
                    <tr>
                        ...
                        <th class="p-4 text-left font-semibold text-text-secondary text-sm border-b-2 border-surface-border">{{ $t('document.warehouse') }}</th>
                ...
                <tbody>
                    ...
                    <tr v-else v-for="document in documentStore.documents" :key="document.id" class="hover:bg-surface-hover">
                        ...
                        <td class="p-4 border-b border-surface-border text-text-primary">{{ document.warehouse?.title || '-' }}</td>
...
const documentType = ref('adjustment');
