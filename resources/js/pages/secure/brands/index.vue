<template>
    <div class="w-full">
        ...
        <Pagination
            v-if="brandStore.pagination && (brandStore.pagination.last_page > 1 || perPage !== 10)"
            :current-page="brandStore.pagination?.current_page || 1"
            :total-pages="brandStore.pagination?.last_page || 1"
            :total-items="brandStore.pagination?.total || 0"
            :per-page="perPage"
            :show-total="true"
...
onMounted(async () => {
    const result = await brandStore.fetchBrands({ per_page: perPage.value });
    if (!result.success) {
