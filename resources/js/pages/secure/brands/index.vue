<template>
    <div class="w-full">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-text-primary m-0">{{ $t('navigation.brands') }}</h1>
        ...
        <div class="bg-bg-elevated rounded-xl overflow-hidden shadow-md border border-surface-border">
            <table class="w-full border-collapse">
                <thead class="bg-bg-secondary">
                    <tr>
                        <th class="p-4 text-left font-semibold text-text-secondary text-sm border-b-2 border-surface-border">{{ $t('brand.code') }}</th>
                        <th class="p-4 text-left font-semibold text-text-secondary text-sm border-b-2 border-surface-border">{{ $t('brand.title') }}</th>
                        <th class="p-4 text-left font-semibold text-text-secondary text-sm border-b-2 border-surface-border">{{ $t('brand.status') }}</th>
                        <th class="p-4 text-left font-semibold text-text-secondary text-sm border-b-2 border-surface-border">{{ $t('common.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="brandStore.loading">
                        <td colspan="4" class="text-center p-10 text-text-tertiary">
                            <span>{{ $t('common.loading') }}</span>
                        </td>
                    </tr>
                    <tr v-else-if="brandStore.brands.length === 0">
                        <td colspan="4" class="text-center p-10 text-text-tertiary">
                            <span>{{ $t('common.noData') }}</span>
                        </td>
                    </tr>
                    <tr v-else v-for="brand in brandStore.brands" :key="brand.id" class="hover:bg-surface-hover">
                        <td class="p-4 border-b border-surface-border text-text-primary">{{ brand.code }}</td>
                        <td class="p-4 border-b border-surface-border text-text-primary">{{ brand.title }}</td>
                        <td class="p-4 border-b border-surface-border text-text-primary">
                            <span :class="[
                                ...
                                {{ brand.status ? $t('common.active') : $t('common.inactive') }}
                            </span>
                        </td>
                        <td class="p-4 border-b border-surface-border text-text-primary">
                            <div class="flex gap-2">
                                <button @click="openEditModal(brand)" class="p-1.5 border-none rounded-md cursor-pointer transition-all duration-200 flex items-center justify-center bg-blue-100 text-accent-info hover:bg-blue-200 dark:bg-blue-900/15 dark:hover:bg-blue-900/25" :title="$t('common.edit')">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                                    </svg>
                                </button>
                                <button @click="handleDelete(brand)" class="p-1.5 border-none rounded-md cursor-pointer transition-all duration-200 flex items-center justify-center bg-red-100 text-accent-error hover:bg-red-200 dark:bg-red-900/15 dark:hover:bg-red-900/25" :title="$t('common.delete')">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div v-if="brandStore.pagination.last_page > 1" class="flex justify-center items-center gap-4 mt-6">
            <button
                @click="changePage(brandStore.pagination.current_page - 1)"
                :disabled="brandStore.pagination.current_page === 1"
            ...
            <span class="text-text-secondary text-sm">
                {{ $t('common.page') }} {{ brandStore.pagination.current_page }} {{ $t('common.of') }} {{ brandStore.pagination.last_page }}
            </span>
            <button
                @click="changePage(brandStore.pagination.current_page + 1)"
                :disabled="brandStore.pagination.current_page === brandStore.pagination.last_page"
                class="px-4 py-2 border-2 border-surface-border rounded-lg bg-bg-primary text-text-primary cursor-pointer transition-all duration-200 hover:border-accent-primary hover:text-accent-primary hover:bg-surface-hover disabled:opacity-50 disabled:cursor-not-allowed"
            >
                {{ $t('common.next') }}
            </button>
...
import BrandModal from './modal.vue';
...
const handleDelete = async (brand) => {
    if (!confirm(t('brand.confirmDelete', { title: brand.title }))) {
        return;
    }

    const result = await brandStore.deleteBrand(brand.id);
    if (result.success) {
        success(t('brand.deleted'));
    } else {
        error(result.error || t('brand.deleteFailed'));
    }
};
</script>
