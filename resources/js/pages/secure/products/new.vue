<template>
    <div class="w-full">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-text-primary m-0">{{ $t('product.create') }}</h1>
        ...
        <div class="bg-bg-elevated rounded-xl shadow-md border border-surface-border p-6">
            <form @submit.prevent="handleSubmit">
                <div class="mb-5">
                    <label class="block text-sm font-medium text-text-secondary mb-2">{{ $t('product.code') }}</label>
                    <input
                        type="text"
                        :value="$t('product.autoGenerated')"
                        disabled
...
const handleSubmit = async () => {
    ...
    if (result.success) {
        ...
        error(result.error || t('product.createFailed'));
        if (result.errors) {
